...
WORKDIR="$HOME/rodinia_3.1/cuda/streamcluster"
RUNSTAMP="$(date +%Y%m%d_%H%M%S)"
OUTDIR="$HOME/thesis/results/streamcluster_kernel_energy/$RUNSTAMP"
...
KERNEL_EXES=(
  "./01_streamcluster_compute_cost_repeat.exe"
)

WORKLOAD_PREFIX="streamcluster"
...
run_case() {
  ...
  set +e
  "$EXE" "$NUM_POINTS" "$DIM" "$NUM_CENTERS" > "$RUN_DIR/program_stdout.txt"
  local exe_rc=$?
  set -e
