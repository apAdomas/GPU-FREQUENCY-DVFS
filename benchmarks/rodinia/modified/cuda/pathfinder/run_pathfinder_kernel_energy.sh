...
WORKDIR="$HOME/rodinia_3.1/cuda/pathfinder"
RUNSTAMP="$(date +%Y%m%d_%H%M%S)"
OUTDIR="$HOME/thesis/results/pathfinder_kernel_energy/$RUNSTAMP"
...
KERNEL_EXES=(
  "./01_pathfinder_dynproc_repeat.exe"
)

WORKLOAD_PREFIX="pathfinder"
...
run_case() {
  ...
  set +e
  "$EXE" "$COLS" "$ROWS" "$PYRAMID_HEIGHT" > "$RUN_DIR/program_stdout.txt"
  local exe_rc=$?
  set -e
