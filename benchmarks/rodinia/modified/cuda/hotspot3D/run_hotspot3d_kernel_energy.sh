...
WORKDIR="$HOME/rodinia_3.1/cuda/hotspot3D"
DATA_DIR="$HOME/rodinia_3.1/data/hotspot3D"
RUNSTAMP="$(date +%Y%m%d_%H%M%S)"
OUTDIR="$HOME/thesis/results/hotspot3d_kernel_energy/$RUNSTAMP"

GRID_SIZE=512
LAYERS=8
ITERATIONS=60

POWER_FILE="$DATA_DIR/power_512x8"
TEMP_FILE="$DATA_DIR/temp_512x8"
...
KERNEL_EXES=(
  "./01_hotspot3d_hotspotOpt1_repeat.exe"
)

WORKLOAD_PREFIX="hotspot3d"
...
[ -f "$POWER_FILE" ] || { echo "Missing power file: $POWER_FILE"; find "$DATA_DIR" -maxdepth 1 -type f || true; exit 1; }
[ -f "$TEMP_FILE" ] || { echo "Missing temp file: $TEMP_FILE"; find "$DATA_DIR" -maxdepth 1 -type f || true; exit 1; }
