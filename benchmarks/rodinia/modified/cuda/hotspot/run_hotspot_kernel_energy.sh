...
WORKDIR="$HOME/rodinia_3.1/cuda/hotspot"
RUNSTAMP="$(date +%Y%m%d_%H%M%S)"
OUTDIR="$HOME/thesis/results/hotspot_kernel_energy/$RUNSTAMP"

GRID_SIZE=512
PYRAMID_HEIGHT=1
SIM_TIME=60
TEMP_FILE="$HOME/rodinia_3.1/data/hotspot/temp_512"
POWER_FILE="$HOME/rodinia_3.1/data/hotspot/power_512"
...
KERNEL_EXES=(
  "./01_hotspot_calculate_temp_repeat.exe"
)

WORKLOAD_PREFIX="hotspot"
...
[ -f "$TEMP_FILE" ] || { echo "Missing temp file: $TEMP_FILE"; exit 1; }
[ -f "$POWER_FILE" ] || { echo "Missing power file: $POWER_FILE"; exit 1; }
...
run_case() {
...

  {
    ...
    echo "pyramid_height=$PYRAMID_HEIGHT"
