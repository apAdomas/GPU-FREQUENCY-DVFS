/**
 * 05_srad_srad2_repeat.cu
 *
 * Isolated measurement for Rodinia SRAD v1 srad2 kernel.
 *
 * Measures:
 *   srad2<<<blocks, threads>>>(lambda, Nr, Nc, Ne, d_iN, d_iS, d_jE, d_jW,
 *                              d_dN, d_dS, d_dE, d_dW, d_c, d_I)
 *
 * Usage:
 *   ./05_srad_srad2_repeat.exe <Nr> <Nc> <lambda>
 *
 * Example:
 *   ./05_srad_srad2_repeat.exe 2048 2048 0.5
 */
 ...
 /*
  * Original SRAD v1 srad2 kernel.
  */
 __global__ void srad2(
     fp d_lambda,
     int d_Nr,
     int d_Nc,
     long d_Ne,
 ...
 static void initialize_image_and_derivatives(
     fp* image,
     fp* dN,
     fp* dS,
     fp* dE,
 ...
 {
     /*
      * Simulate state after srad:
      * - image is positive
      * - derivatives are small signed values
      * - diffusion coefficient c is in [0, 1]
      */
     for (long i = 0; i < Ne; i++)
     {
         image[i] = expf((1.0f + (fp)(i % 255)) / 255.0f);
 
         dN[i] = ((fp)((i % 17) - 8)) * 0.001f;
         dS[i] = ((fp)((i % 19) - 9)) * 0.001f;
         dE[i] = ((fp)((i % 23) - 11)) * 0.001f;
         dW[i] = ((fp)((i % 29) - 14)) * 0.001f;
 
         c[i] = 0.1f + ((fp)(i % 90) / 100.0f);
     }
 }
     ...
     int blocks_x)
 {
     ...
     checkCuda(cudaGetLastError(), "launch srad_srad2");
 }
 ...
 int main(int argc, char** argv)
 {
     ...
     printf(
         "Loaded SRAD v1 config: kernel=srad2 Nr=%d Nc=%d Ne=%ld lambda=%.6f blocks=%d threads=%d\n",
         Nr,
         Nc,
         Ne,
         lambda,
     ...
     fp* h_dN = (fp*)malloc(sizeof(fp) * (size_t)Ne);
     fp* h_dS = (fp*)malloc(sizeof(fp) * (size_t)Ne);
     fp* h_dE = (fp*)malloc(sizeof(fp) * (size_t)Ne);
     fp* h_dW = (fp*)malloc(sizeof(fp) * (size_t)Ne);
     fp* h_c = (fp*)malloc(sizeof(fp) * (size_t)Ne);
     ...
     initialize_image_and_derivatives(
         image,
         h_dN,
         h_dS,
         h_dE,
     ...
     /*
      * Reset before measurement because srad2 mutates d_I.
      */
     reset_device_state(
         d_I,
         d_dN,
