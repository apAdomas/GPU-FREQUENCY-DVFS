/**
 * 01_srad_extract_repeat.cu
 *
 * Isolated measurement for Rodinia SRAD v1 extract kernel.
 *
 * Measures:
 *   extract<<<blocks, threads>>>(Ne, d_I)
 *
 * Usage:
 *   ./01_srad_extract_repeat.exe <Nr> <Nc>
 *
 * Example:
 *   ./01_srad_extract_repeat.exe 2048 2048
 */
 ...
 /*
  * Original SRAD v1 extract kernel.
  */
 ...
 {
     /*
      * Deterministic synthetic image in roughly 0..255 range.
      * Avoid 0 because later SRAD kernels often divide by image values.
      */
     for (long i = 0; i < Ne; i++)
     {
         image[i] = 1.0f + (fp)(i % 255);
     }
 }
 ...
 static void launch_extract_once(
     fp* d_I,
     long Ne,
     int blocks_x)
 {
     ...
     checkCuda(cudaGetLastError(), "launch srad_extract");
 }
 ...
 int main(int argc, char** argv)
 {
     ...
     printf(
         "Loaded SRAD v1 config: kernel=extract Nr=%d Nc=%d Ne=%ld blocks=%d threads=%d\n",
         Nr,
         Nc,
         Ne,
         blocks_x,
     ...
     /*
      * Reset before measurement because extract mutates d_I.
      */
     reset_device_state(
         d_I,
         image,
