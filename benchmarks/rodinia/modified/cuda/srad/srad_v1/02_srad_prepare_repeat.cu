/**
 * 02_srad_prepare_repeat.cu
 *
 * Isolated measurement for Rodinia SRAD v1 prepare kernel.
 *
 * Measures:
 *   prepare<<<blocks, threads>>>(Ne, d_I, d_sums, d_sums2)
 *
 * Usage:
 *   ./02_srad_prepare_repeat.exe <Nr> <Nc>
 *
 * Example:
 *   ./02_srad_prepare_repeat.exe 2048 2048
 */
 ...
 /*
  * Original SRAD v1 prepare kernel.
  */
 ...
 {
     /*
      * Simulate image after extract: positive values around exp(0..1).
      */
     for (long i = 0; i < Ne; i++)
     {
     ...
     long Ne)
 {
     ...
     checkCuda(
         cudaMemset(d_sums, 0, sizeof(fp) * (size_t)Ne),
         "memset d_sums");
 
     checkCuda(
         cudaMemset(d_sums2, 0, sizeof(fp) * (size_t)Ne),
         "memset d_sums2");
 
     checkCuda(cudaDeviceSynchronize(), "sync after reset_device_state");
 }
 
 static void launch_prepare_once(
     fp* d_I,
     fp* d_sums,
     fp* d_sums2,
     long Ne,
     int blocks_x)
 {
     ...
     checkCuda(cudaGetLastError(), "launch srad_prepare");
 }
 ...
 int main(int argc, char** argv)
 {
     ...
     printf(
         "Loaded SRAD v1 config: kernel=prepare Nr=%d Nc=%d Ne=%ld blocks=%d threads=%d\n",
         Nr,
         Nc,
         Ne,
         blocks_x,
     ...
     /*
      * Reset before measurement because prepare mutates d_sums and d_sums2.
      */
     reset_device_state(
         d_I,
         d_sums,
