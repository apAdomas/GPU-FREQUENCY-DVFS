...
WORKDIR="$HOME/rodinia_3.1/cuda/srad/srad_v1"
RUNSTAMP="$(date +%Y%m%d_%H%M%S)"
OUTDIR="$HOME/thesis/results/srad_v1_kernel_energy/$RUNSTAMP"
...
KERNEL_EXES=(
  "./01_srad_extract_repeat.exe"
  "./02_srad_prepare_repeat.exe"
  "./03_srad_reduce_repeat.exe"
  "./04_srad_srad_repeat.exe"
  "./05_srad_srad2_repeat.exe"
  "./06_srad_compress_repeat.exe"
)

WORKLOAD_PREFIX="srad_v1"
...
run_case() {
  ...
  set +e
  if [ "$KERNEL" = "srad" ] || [ "$KERNEL" = "srad2" ]; then
    "$EXE" "$NR" "$NC" "$LAMBDA" > "$RUN_DIR/program_stdout.txt"
  else
    "$EXE" "$NR" "$NC" > "$RUN_DIR/program_stdout.txt"
  fi
  local exe_rc=$?
  set -e
