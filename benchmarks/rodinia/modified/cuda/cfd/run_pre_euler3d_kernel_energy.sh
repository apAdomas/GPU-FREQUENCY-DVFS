...
WORKDIR="$HOME/rodinia_3.1/cuda/cfd"
RUNSTAMP="$(date +%Y%m%d_%H%M%S)"
OUTDIR="$HOME/thesis/results/pre_euler3d_kernel_energy/$RUNSTAMP"

CFD_INPUT="$HOME/rodinia_3.1/data/cfd/fvcorr.domn.097K"
...
KERNEL_EXES=(
  "./01_pre_euler3d_compute_step_factor_repeat.exe"
  "./02_pre_euler3d_compute_flux_contributions_repeat.exe"
  "./03_pre_euler3d_compute_flux_repeat.exe"
  "./04_pre_euler3d_time_step_repeat.exe"
)

WORKLOAD_PREFIX="pre_euler3d"
...
[ -f "$CFD_INPUT" ] || { echo "Missing CFD input: $CFD_INPUT"; exit 1; }
...
run_case() {
  ...
  "$EXE" "$CFD_INPUT" > "$RUN_DIR/program_stdout.txt"
  ...
  for REP in $(seq 1 "$REPEATS"); do
    ...
    for CORE in "${CORE_CLOCKS[@]}"; do
      for MEM in "${MEM_CLOCKS[@]}"; do
        run_case "$KERNEL" "$EXE" "$CORE" "$MEM" "$REP"
      done
    done
