...
WORKDIR="$HOME/rodinia_3.1/cuda/particlefilter"
RUNSTAMP="$(date +%Y%m%d_%H%M%S)"
OUTDIR="$HOME/thesis/results/particlefilter_kernel_energy/$RUNSTAMP"
...
KERNEL_EXES=(
  "./01_particlefilter_likelihood_repeat.exe"
  "./02_particlefilter_sum_repeat.exe"
  "./03_particlefilter_normalize_weights_repeat.exe"
  "./04_particlefilter_find_index_repeat.exe"
)

WORKLOAD_PREFIX="particlefilter"
...
run_case() {
  ...
  set +e
  "$EXE" "$ISZX" "$ISZY" "$NFR" "$NPARTICLES" > "$RUN_DIR/program_stdout.txt"
  local exe_rc=$?
  set -e
