/**
 * 02_3mm_kernel2_repeat.cu
 *
 * Isolated measurement for mm3_kernel2.
 *
 * Measures a single mm3_kernel2 launch
...
static void launch_kernel2_once(
    ...
    dim3 block)
{
    mm3_kernel2<<<grid, block>>>(ni, nj, nk, nl, nm, C_gpu, D_gpu, F_gpu);
    checkCuda(cudaGetLastError(), "launch mm3_kernel2");
}

void mm3CudaKernel2Only(
    int ni,
    int nj,
    int nk,
    int nl,
    ...
    DATA_TYPE POLYBENCH_2D(F_outputFromGpu, NJ, NL, nj, nl))
{
    DATA_TYPE *C_gpu = NULL;
    DATA_TYPE *D_gpu = NULL;
    DATA_TYPE *F_gpu = NULL;

    checkCuda(cudaMalloc((void **)&C_gpu, sizeof(DATA_TYPE) * NJ * NM), "cudaMalloc C_gpu");
    checkCuda(cudaMalloc((void **)&D_gpu, sizeof(DATA_TYPE) * NM * NL), "cudaMalloc D_gpu");
    ...
    copy_inputs_to_device(C_gpu, D_gpu, F_gpu, C, D, F);
    ...
    int warmup_launches = 0;
    {
        ...
        {
            launch_kernel2_once(ni, nj, nk, nl, nm, C_gpu, D_gpu, F_gpu, grid, block);
...

    {
        ...
        printf("RESULT kernel=mm3_kernel2\n");
...
int main()
{
    ...
    POLYBENCH_2D_ARRAY_DECL(F_outputFromGpu, DATA_TYPE, NJ, NL, nj, nl);
    ...
    memset(POLYBENCH_ARRAY(F), 0, sizeof(DATA_TYPE) * NJ * NL);
    ...
    mm3CudaKernel2Only(
        ...
        POLYBENCH_ARRAY(D),
        POLYBENCH_ARRAY(F),
        POLYBENCH_ARRAY(F_outputFromGpu));
    ...
    POLYBENCH_FREE_ARRAY(F_outputFromGpu);
