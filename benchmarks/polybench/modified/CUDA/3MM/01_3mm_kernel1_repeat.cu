/**
 * 01_3mm_kernel1_repeat.cu
 *
 * Isolated measurement for mm3_kernel1.
 *
 * Measures a single mm3_kernel1 launch
...
static void launch_kernel1_once(
    ...
    dim3 block)
{
    mm3_kernel1<<<grid, block>>>(ni, nj, nk, nl, nm, A_gpu, B_gpu, E_gpu);
    checkCuda(cudaGetLastError(), "launch mm3_kernel1");
}

void mm3CudaKernel1Only(
    int ni,
    int nj,
    int nk,
    int nl,
    ...
    DATA_TYPE POLYBENCH_2D(E_outputFromGpu, NI, NJ, ni, nj))
{
    ...
    copy_inputs_to_device(A_gpu, B_gpu, E_gpu, A, B, E);
    ...
    int warmup_launches = 0;
    {
        ...
        {
            launch_kernel1_once(ni, nj, nk, nl, nm, A_gpu, B_gpu, E_gpu, grid, block);
...

    {
        ...
        printf("RESULT kernel=mm3_kernel1\n");
...
int main()
{
    ...
    POLYBENCH_2D_ARRAY_DECL(E_outputFromGpu, DATA_TYPE, NI, NJ, ni, nj);
    ...
    memset(POLYBENCH_ARRAY(E), 0, sizeof(DATA_TYPE) * NI * NJ);
    ...
    mm3CudaKernel1Only(
        ...
        POLYBENCH_ARRAY(B),
        POLYBENCH_ARRAY(E),
        POLYBENCH_ARRAY(E_outputFromGpu));
