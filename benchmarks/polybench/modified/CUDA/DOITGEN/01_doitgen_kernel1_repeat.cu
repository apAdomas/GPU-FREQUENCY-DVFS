/**
 * 01_doitgen_kernel1_repeat.cu
 *
 * Isolated measurement for doitgen_kernel1.
 *
 * Measures a single doitgen_kernel1 launch (at fixed r = FIXED_R)
...
static void launch_kernel1_once(
    ...
    dim3 block)
{
    doitgen_kernel1<<<grid, block>>>(sum_gpu, A_gpu, C4_gpu, FIXED_R);
    checkCuda(cudaGetLastError(), "launch kernel1");
}

void doitgenCudaKernel1Only(
    DATA_TYPE* A,
    DATA_TYPE* C4,
    DATA_TYPE* sum,
    DATA_TYPE* sum_outputFromGpu)
{
    ...
    int warmup_launches = 0;
    {
        ...
        {
            launch_kernel1_once(sum_gpu, A_gpu, C4_gpu, grid, block);
...

    {
        ...
        printf("RESULT kernel=doitgen_kernel1\n");
...
int main()
{
    ...
    DATA_TYPE* sum_outputFromGpu = (DATA_TYPE*)malloc(NR * NQ * NP * sizeof(DATA_TYPE));

    if (!A || !C4 || !sum || !sum_outputFromGpu)
    {
        fprintf(stderr, "Host allocation failed\n");
        return 1;
    }

    memset(sum, 0, NR * NQ * NP * sizeof(DATA_TYPE));

    init_array(A, C4);

    GPU_argv_init_measurement();

    doitgenCudaKernel1Only(
        A,
        C4,
        sum,
        sum_outputFromGpu);
