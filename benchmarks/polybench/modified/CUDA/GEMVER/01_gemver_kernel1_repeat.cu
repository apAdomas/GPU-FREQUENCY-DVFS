/**
 * 01_gemver_kernel1_repeat.cu
 *
 * Isolated measurement for gemver_kernel1.
 *
 * Measures a single gemver_kernel1 launch
...
__global__ void gemver_kernel1(
    int n,
    DATA_TYPE alpha,
    DATA_TYPE beta,
    DATA_TYPE *a,
...
static void launch_kernel1_once(
    ...
    dim3 block)
{
    gemver_kernel1<<<grid, block>>>(n, alpha, beta, A_gpu, v1_gpu, v2_gpu, u1_gpu, u2_gpu);
    checkCuda(cudaGetLastError(), "launch gemver_kernel1");
}

void gemverCudaKernel1Only(
    int n,
    DATA_TYPE alpha,
    DATA_TYPE beta,
    DATA_TYPE POLYBENCH_2D(A, N, N, n, n),
    ...
    DATA_TYPE POLYBENCH_2D(A_outputFromGpu, N, N, n, n))
{
    ...
    DATA_TYPE *u1_gpu = NULL;
    DATA_TYPE *v1_gpu = NULL;
    DATA_TYPE *u2_gpu = NULL;
    DATA_TYPE *v2_gpu = NULL;

    checkCuda(cudaMalloc((void **)&A_gpu, sizeof(DATA_TYPE) * N * N), "cudaMalloc A_gpu");
    checkCuda(cudaMalloc((void **)&u1_gpu, sizeof(DATA_TYPE) * N), "cudaMalloc u1_gpu");
    checkCuda(cudaMalloc((void **)&v1_gpu, sizeof(DATA_TYPE) * N), "cudaMalloc v1_gpu");
    checkCuda(cudaMalloc((void **)&u2_gpu, sizeof(DATA_TYPE) * N), "cudaMalloc u2_gpu");
    checkCuda(cudaMalloc((void **)&v2_gpu, sizeof(DATA_TYPE) * N), "cudaMalloc v2_gpu");

    dim3 block(DIM_THREAD_BLOCK_KERNEL_1_X, DIM_THREAD_BLOCK_KERNEL_1_Y);
    dim3 grid(
        (size_t)ceil(((float)N) / ((float)DIM_THREAD_BLOCK_KERNEL_1_X)),
        (size_t)ceil(((float)N) / ((float)DIM_THREAD_BLOCK_KERNEL_1_Y)));
    ...
    copy_inputs_to_device(A_gpu, u1_gpu, v1_gpu, u2_gpu, v2_gpu, A, u1, v1, u2, v2);
    ...
    int warmup_launches = 0;
    {
        ...
        {
            launch_kernel1_once(n, alpha, beta, A_gpu, v1_gpu, v2_gpu, u1_gpu, u2_gpu, grid, block);
...

    {
        ...
        printf("RESULT kernel=gemver_kernel1\n");
    ...
    checkCuda(cudaFree(u1_gpu), "cudaFree u1_gpu");
    checkCuda(cudaFree(v1_gpu), "cudaFree v1_gpu");
    checkCuda(cudaFree(u2_gpu), "cudaFree u2_gpu");
    checkCuda(cudaFree(v2_gpu), "cudaFree v2_gpu");
}
