/**
 * 02_gemver_kernel2_repeat.cu
 *
 * Isolated measurement for gemver_kernel2.
 *
 * Measures a single gemver_kernel2 launch
...
__global__ void gemver_kernel2(
    int n,
    DATA_TYPE alpha,
    DATA_TYPE beta,
    DATA_TYPE *a,
...
static void launch_kernel2_once(
    ...
    dim3 block)
{
    gemver_kernel2<<<grid, block>>>(n, alpha, beta, A_gpu, x_gpu, y_gpu, z_gpu);
    checkCuda(cudaGetLastError(), "launch gemver_kernel2");
}

void gemverCudaKernel2Only(
    int n,
    DATA_TYPE alpha,
    DATA_TYPE beta,
    DATA_TYPE POLYBENCH_2D(A, N, N, n, n),
    ...
    DATA_TYPE POLYBENCH_1D(y, N, n),
    DATA_TYPE POLYBENCH_1D(z, N, n),
    DATA_TYPE POLYBENCH_1D(x_outputFromGpu, N, n))
{
    DATA_TYPE *A_gpu = NULL;
    DATA_TYPE *x_gpu = NULL;
    DATA_TYPE *y_gpu = NULL;
    DATA_TYPE *z_gpu = NULL;
    ...
    checkCuda(cudaMalloc((void **)&z_gpu, sizeof(DATA_TYPE) * N), "cudaMalloc z_gpu");
    ...
    copy_inputs_to_device(A_gpu, x_gpu, y_gpu, z_gpu, A, x, y, z);
    ...
    int warmup_launches = 0;
    {
        ...
        {
            launch_kernel2_once(n, alpha, beta, A_gpu, x_gpu, y_gpu, z_gpu, grid, block);
...

    {
        ...
        printf("RESULT kernel=gemver_kernel2\n");
    ...
    checkCuda(cudaFree(z_gpu), "cudaFree z_gpu");
}
...
int main()
{
    ...
    POLYBENCH_1D_ARRAY_DECL(x_outputFromGpu, DATA_TYPE, N, n);
