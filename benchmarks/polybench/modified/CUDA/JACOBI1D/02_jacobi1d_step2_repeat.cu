/**
 * 02_jacobi1d_step2_repeat.cu
 *
 * Isolated measurement for runJacobiCUDA_kernel2.
 *
 * Protocol inside this executable:
 * 1) allocate once
 * 2) copy/reset inputs to device
 * 3) warm up isolated kernel2 region for WARMUP_SECONDS
 * 4) reset inputs again
 * 5) measure isolated kernel2 region for MEASURE_SECONDS
...
void runJacobi1DCUDAKernel2Only(
    int n,
    DATA_TYPE POLYBENCH_1D(A, N, n),
    DATA_TYPE POLYBENCH_1D(B, N, n),
    DATA_TYPE POLYBENCH_1D(A_outputFromGpu, N, n))
{
    ...
    int warmup_launches = 0;
    {
        ...
        while ((now_seconds() - warmup_start) < WARMUP_SECONDS) {
            launch_kernel2_once(n, Agpu, Bgpu, grid, block);
    ...
    checkCuda(cudaMemcpy(A_outputFromGpu, Agpu, sizeof(DATA_TYPE) * n, cudaMemcpyDeviceToHost),
...

    {
        ...
        printf("RESULT kernel=jacobi1d_kernel2\n");
