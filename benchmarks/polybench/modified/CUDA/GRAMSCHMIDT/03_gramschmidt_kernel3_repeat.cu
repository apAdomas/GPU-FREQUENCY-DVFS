/**
 * 03_gramschmidt_kernel3_repeat.cu
 *
 * Isolated measurement for gramschmidt_kernel3.
 *
 * Measures a single gramschmidt_kernel3 launch (at fixed k = FIXED_K)
...
__global__ void gramschmidt_kernel3(
    int ni,
    int nj,
    DATA_TYPE *a,
    DATA_TYPE *r,
...
static void launch_kernel3_once(
    ...
    dim3 block)
{
    gramschmidt_kernel3<<<grid, block>>>(
        ni,
        nj,
        A_gpu,
        R_gpu,
    ...
    checkCuda(cudaGetLastError(), "launch kernel3");
}

void gramschmidtCudaKernel3Only(
    int ni,
    int nj,
    DATA_TYPE POLYBENCH_2D(A,NI,NJ,ni,nj),
    DATA_TYPE POLYBENCH_2D(R,NJ,NJ,nj,nj),
    ...
    DATA_TYPE POLYBENCH_2D(A_outputFromGpu,NI,NJ,ni,nj))
{
...

    {
        ...
        printf("RESULT kernel=gramschmidt_kernel3\n");
