/**
 * 01_gramschmidt_kernel1_repeat.cu
 *
 * Isolated measurement for gramschmidt_kernel1.
 *
 * Measures a single gramschmidt_kernel1 launch (at fixed k = FIXED_K)
...
__global__ void gramschmidt_kernel1(
    int ni,
    int nj,
    DATA_TYPE *a,
    DATA_TYPE *r,
...
static void launch_kernel1_once(
    ...
    dim3 block)
{
    gramschmidt_kernel1<<<grid, block>>>(
        ni,
        nj,
        A_gpu,
        R_gpu,
...
void gramschmidtCudaKernel1Only(
    int ni,
    int nj,
    DATA_TYPE POLYBENCH_2D(A,NI,NJ,ni,nj),
    DATA_TYPE POLYBENCH_2D(R,NJ,NJ,nj,nj),
    DATA_TYPE POLYBENCH_2D(Q,NI,NJ,ni,nj),
    DATA_TYPE POLYBENCH_2D(R_outputFromGpu,NJ,NJ,nj,nj))
{
...

    {
        ...
        printf("RESULT kernel=gramschmidt_kernel1\n");
...
int main()
{
    ...
    POLYBENCH_FREE_ARRAY(R_outputFromGpu);
