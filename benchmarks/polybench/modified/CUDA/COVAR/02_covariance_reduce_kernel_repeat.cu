/**
 * 02_covariance_reduce_kernel_repeat.cu
 *
...
static void launch_reduce_once(
    ...
    dim3 block)
{
    reduce_kernel<<<grid, block>>>(m, n, mean_gpu, data_gpu);
    checkCuda(cudaGetLastError(), "launch reduce_kernel");
}

void covarianceCudaReduceKernelOnly(
    int m,
    int n,
    DATA_TYPE POLYBENCH_2D(data, M, N, m, n),
    ...
    DATA_TYPE POLYBENCH_2D(data_outputFromGpu, M, N, m, n))
{
    ...
    dim3 grid(
        ...
        (size_t)ceil(((float)N) / ((float)DIM_THREAD_BLOCK_KERNEL_2_Y)));
    ...
    int warmup_launches = 0;
    {
        ...
        {
            launch_reduce_once(m, n, mean_gpu, data_gpu, grid, block);
...
int main()
{
    ...
    covarianceCudaReduceKernelOnly(
        m,
        n,
