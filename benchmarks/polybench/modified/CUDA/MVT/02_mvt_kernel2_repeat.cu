/**
 * 02_mvt_kernel2_repeat.cu
 *
 * Isolated measurement for mvt_kernel2.
 *
 * Measures a single mvt_kernel2 launch
...
static void launch_kernel2_once(
    ...
    dim3 block)
{
    mvt_kernel2<<<grid, block>>>(n, a_gpu, x2_gpu, y2_gpu);
    checkCuda(cudaGetLastError(), "launch kernel2");
}

void runMvtCudaKernel2Only(
    int n,
    DATA_TYPE POLYBENCH_2D(a, N, N, n, n),
    DATA_TYPE POLYBENCH_1D(x2, N, n),
    DATA_TYPE POLYBENCH_1D(y2, N, n),
    DATA_TYPE POLYBENCH_1D(x2_outputFromGpu, N, n))
{
    ...
    DATA_TYPE* y2_gpu = NULL;

    checkCuda(cudaMalloc((void**)&a_gpu, sizeof(DATA_TYPE) * N * N), "cudaMalloc a_gpu");
    checkCuda(cudaMalloc((void**)&x2_gpu, sizeof(DATA_TYPE) * N), "cudaMalloc x2_gpu");
    checkCuda(cudaMalloc((void**)&y2_gpu, sizeof(DATA_TYPE) * N), "cudaMalloc y2_gpu");
    ...
    copy_inputs_to_device(a_gpu, x2_gpu, y2_gpu, a, x2, y2);
    ...
    int warmup_launches = 0;
    {
        ...
        {
            launch_kernel2_once(n, a_gpu, x2_gpu, y2_gpu, grid, block);
...

    {
        ...
        printf("RESULT kernel=mvt_kernel2\n");
    ...
    checkCuda(cudaFree(x2_gpu), "cudaFree x2_gpu");
    checkCuda(cudaFree(y2_gpu), "cudaFree y2_gpu");
}
