/**
 * 02_atax_kernel2_repeat.cu
 *
 * Isolated measurement for atax_kernel2.
 *
 * Measures a single atax_kernel2 launch
...
static void compute_tmp_on_host(
    int nx,
    int ny,
    DATA_TYPE (*A)[NY],
    DATA_TYPE* x,
...
static void launch_kernel2_once(
    ...
    dim3 block)
{
    atax_kernel2<<<grid, block>>>(nx, ny, A_gpu, y_gpu, tmp_gpu);
    checkCuda(cudaGetLastError(), "launch atax_kernel2");
}

void ataxGpuKernel2Only(
    int nx,
    int ny,
    DATA_TYPE POLYBENCH_2D(A, NX, NY, nx, ny),
    DATA_TYPE POLYBENCH_1D(x, NY, ny),
    ...
    DATA_TYPE POLYBENCH_1D(y_outputFromGpu, NY, ny))
{
    ...
    checkCuda(cudaMalloc((void **)&y_gpu, sizeof(DATA_TYPE) * NY), "cudaMalloc y_gpu");
    ...
    compute_tmp_on_host(nx, ny, A, x, tmp);
    ...
    copy_inputs_to_device(A_gpu, y_gpu, tmp_gpu, A, y, tmp);
    ...
    int warmup_launches = 0;
    {
        ...
        {
            launch_kernel2_once(nx, ny, A_gpu, y_gpu, tmp_gpu, grid, block);
...

    {
        ...
        printf("RESULT kernel=atax_kernel2\n");
