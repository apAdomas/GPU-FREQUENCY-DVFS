/**
 * 01_syr2k_kernel_repeat.cu
 *
 * Isolated measurement for syr2k_kernel.
 *
 * Measures a single syr2k_kernel launch
    ...
    dim3 block)
{
    ...
    checkCuda(cudaGetLastError(), "launch syr2k_kernel");
}

void syr2kCudaKernelOnly(
    int ni,
    int nj,
    DATA_TYPE alpha,
    DATA_TYPE beta,
    ...
    DATA_TYPE POLYBENCH_2D(C_outputFromGpu,NI,NI,ni,ni))
{
    ...
    int warmup_launches = 0;
    {
        ...
        {
            launch_kernel_once(ni, nj, alpha, beta, A_gpu, B_gpu, C_gpu, grid, block);
...

    {
        ...
        printf("RESULT kernel=syr2k_kernel\n");
