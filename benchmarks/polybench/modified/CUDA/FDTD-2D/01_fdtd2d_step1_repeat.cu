/**
 * 01_fdtd2d_step1_repeat.cu
 *
 * Isolated measurement for fdtd_step1_kernel.
 *
 * Protocol inside this executable:
 * 1) allocate once
 * 2) copy/reset inputs to device
 * 3) warm up isolated step1 region for WARMUP_SECONDS
 * 4) reset inputs again
 * 5) measure isolated step1 region for MEASURE_SECONDS
...
static void launch_step1_once(
    ...
    dim3 block)
{
    fdtd_step1_kernel<<<grid, block>>>(nx, ny, _fict_gpu, ex_gpu, ey_gpu, hz_gpu, 0);
    checkCuda(cudaGetLastError(), "launch step1");
}

void fdtdCudaStep1Only(
    int tmax,
    int nx,
    int ny,
    DATA_TYPE POLYBENCH_1D(_fict_, TMAX, TMAX),
    ...
    DATA_TYPE POLYBENCH_2D(ey, NX, NY, nx, ny),
    DATA_TYPE POLYBENCH_2D(hz, NX, NY, nx, ny),
    DATA_TYPE POLYBENCH_2D(ey_outputFromGpu, NX, NY, nx, ny))
{
    ...
    checkCuda(cudaMemcpy(ey_outputFromGpu, ey_gpu, sizeof(DATA_TYPE) * NX * NY, cudaMemcpyDeviceToHost),
              "copy ey_outputFromGpu");
...
int main()
{
    ...
    POLYBENCH_2D_ARRAY_DECL(ey_outputFromGpu, DATA_TYPE, NX, NY, nx, ny);
    ...
    fdtdCudaStep1Only(
        ...
        POLYBENCH_ARRAY(ey),
        POLYBENCH_ARRAY(hz),
        POLYBENCH_ARRAY(ey_outputFromGpu));

    POLYBENCH_FREE_ARRAY(_fict_);
    POLYBENCH_FREE_ARRAY(ex);
    POLYBENCH_FREE_ARRAY(ey);
    POLYBENCH_FREE_ARRAY(hz);
    POLYBENCH_FREE_ARRAY(ey_outputFromGpu);
