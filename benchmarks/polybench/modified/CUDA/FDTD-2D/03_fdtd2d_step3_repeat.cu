/**
 * 03_fdtd2d_step3_repeat.cu
 *
 * Isolated measurement for fdtd_step3_kernel.
 *
 * Protocol inside this executable:
 * 1) allocate once
 * 2) copy/reset inputs to device
 * 3) warm up isolated step3 region for WARMUP_SECONDS
 * 4) reset inputs again
 * 5) measure isolated step3 region for MEASURE_SECONDS
...
static void launch_step3_once(
    ...
    dim3 block)
{
    fdtd_step3_kernel<<<grid, block>>>(nx, ny, ex_gpu, ey_gpu, hz_gpu, 0);
    checkCuda(cudaGetLastError(), "launch step3");
}

void fdtdCudaStep3Only(
    int tmax,
    int nx,
    int ny,
    DATA_TYPE POLYBENCH_1D(_fict_, TMAX, TMAX),
    ...
    DATA_TYPE POLYBENCH_2D(hz_outputFromGpu, NX, NY, nx, ny))
{
    ...
    int warmup_launches = 0;
    {
        ...
        while ((now_seconds() - warmup_start) < WARMUP_SECONDS) {
            launch_step3_once(nx, ny, ex_gpu, ey_gpu, hz_gpu, grid, block);
    ...
    checkCuda(cudaMemcpy(hz_outputFromGpu, hz_gpu, sizeof(DATA_TYPE) * NX * NY, cudaMemcpyDeviceToHost),
              "copy hz_outputFromGpu");
