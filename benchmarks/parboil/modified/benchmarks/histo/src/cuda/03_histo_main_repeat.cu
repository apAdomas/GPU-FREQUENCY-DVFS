/**
 * 03_histo_main_repeat.cu
 *
 * Isolated measurement for the Parboil histo main kernel
 * (histo_main_kernel).
 *
 ...
 * 3) run the upstream prescan + intermediates kernels ONCE to produce
 *    valid ranges and sm_mappings (the inputs of the main kernel)
 * 4) warm up isolated main launches for WARMUP_SECONDS
 * 5) measure isolated main launches for MEASURE_SECONDS
 *    - CUDA events for time
 *    - NVML total energy for energy
 * 6) exit
 *
 * Only histo_main_kernel is inside the measured region. The grid.y of the
 * main kernel depends on the prescan-derived range, exactly as in main.cu.
 */
...
int main(int argc, char **argv)
{
    ...
    // --- One-time setup: produce ranges (prescan) and sm_mappings (intermediates) ---
    unsigned int ranges_h[2] = {UINT32_MAX, 0};
    ...
    // Grid for the main kernel depends on the prescan-derived range.
    dim3 main_grid(BLOCK_X, ranges_h[1] - ranges_h[0] + 1);
    dim3 main_block(THREADS);
    unsigned int num_elements = h.img_height * h.img_width;
    ...
    int warmup_launches = 0;
    {
        ...
        while ((now_seconds() - warmup_start) < WARMUP_SECONDS) {
            histo_main_kernel<<<main_grid, main_block>>>(
                d_sm_mappings, num_elements, ranges_h[0], ranges_h[1],
                h.histo_height, h.histo_width,
                d_global_subhisto, (unsigned int *)d_global_histo, d_global_overflow);
            checkCuda(cudaGetLastError(), "launch main (warmup)");
...

    {
        ...
        while ((now_seconds() - measure_wall_start) < MEASURE_SECONDS) {
            histo_main_kernel<<<main_grid, main_block>>>(
                ...
                h.histo_height, h.histo_width,
                d_global_subhisto, (unsigned int *)d_global_histo, d_global_overflow);
            checkCuda(cudaGetLastError(), "launch main (measured)");
...

    {
        ...
        printf("RESULT kernel=parboil_histo_main\n");
