/**
 * 01_histo_prescan_repeat.cu
 *
 * Isolated measurement for the Parboil histo prescan kernel
 * (histo_prescan_kernel).
 *
 ...
 * 3) warm up isolated prescan launches for WARMUP_SECONDS
 * 4) measure isolated prescan launches for MEASURE_SECONDS
 *    - CUDA events for time
 *    - NVML total energy for energy
 * 5) exit
 *
 * Clocks / global warmup are handled outside this executable.
 */
...
static void launch_prescan_once(unsigned int *d_input, int size, unsigned int *d_ranges)
{
    histo_prescan_kernel<<<PRESCAN_BLOCKS_X, PRESCAN_THREADS>>>(d_input, size, d_ranges);
    checkCuda(cudaGetLastError(), "launch prescan");
}
...
int main(int argc, char **argv)
{
    ...
    int warmup_launches = 0;
    {
        ...
        while ((now_seconds() - warmup_start) < WARMUP_SECONDS) {
            launch_prescan_once(d_input, size, d_ranges);
...

    {
        ...
        printf("RESULT kernel=parboil_histo_prescan\n");
